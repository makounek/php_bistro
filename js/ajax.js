...
$(function(){
	$('#submit_reg').click(function(){
		...
		if ((name == "") || (email == "") || (password == "") || (con_password == "")) {
			...
			$('#reg_error').text("Prázdné kolonky!");
		}
		else if (!mail_regex.test(email)) {

			$('#reg_error').text('Email je ve špatném tvaru!');
		}
		else if (!name_regex.test(name)) {

			$('#reg_error').text('Jméno je ve špatném tvaru!');
		}
		else if (password != con_password) {

			$('#reg_error').text("Zadaná hesla nejsou stejná!");
		} else {
			...
			$.ajax({
	            ...
	            success  :function(data){
	            	...
	            	if (data['code'] == "0") {
	            		...
	            		toggleModal('Chyba!', data['msg']);

	            	} else if (data['code'] == "1") {

	            		toggleModal('Úspěch!', data['msg']);
