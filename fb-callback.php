...
$query=$pdoconn->prepare('SELECT * FROM users WHERE facebook_id=:facebookId LIMIT 1;');
$query->execute([
    ':facebookId'=>$fbUserId
]);
...
if ($query->rowCount()>0){
    ...
    $query = $pdoconn->prepare('SELECT * FROM users WHERE email=:email LIMIT 1;');
    $query->execute([
        ':email'=>$fbUserEmail
    ]);
    ...
    if ($query->rowCount()>0){
        ...
        $updateQuery = $pdoconn->prepare('UPDATE users SET facebook_id=:facebookId WHERE user_id=:id LIMTI 1;');
        $updateQuery->execute([
            ':facebookId'=>$fbUserId,
            ':id'=>$user['user_id']
        ]);
        ...
        $insertQuery = $pdoconn->prepare('INSERT INTO users (name, email, facebook_id) VALUES (:name, :email, :facebookId);');
        $insertQuery->execute([
            ':name'=>$fbUserName,
            ':email'=>$fbUserEmail,
            ':facebookId'=>$fbUserId
